...
import 'package:elkhlod/Features/ezen/presentation/widgets/tap_bar_ezen_view_body.dart';
...
class StatusRequestEzen extends StatefulWidget {
  const StatusRequestEzen({super.key});

  @override
  State<StatusRequestEzen> createState() => _StatusRequestEzenState();
}

class _StatusRequestEzenState extends State<StatusRequestEzen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: FadeIn(child: const TapBarEzenViewBody()),
      floatingActionButton: BlocBuilder<GetProfileCubit, GetProfileState>(
        builder: (context, state) {
          if (state is GetProfileSuccessful) {
            return state.data.data.addEzn == "yes"
                ? MaterialButton(
                    onPressed: () {
                      BlocProvider.of<BottomNavCubit>(context)
                          .updateBottomNavIndex(kRequestPermissionScreen);
                      BlocProvider.of<BottomNavCubit>(context)
                          .navigationQueue
