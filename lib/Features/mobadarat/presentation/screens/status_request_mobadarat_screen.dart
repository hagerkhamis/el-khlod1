...
import 'package:elkhlod/Features/mobadarat/presentation/widgets/tap_bar_view_body.dart';
...
class StatusMobadaratRequest extends StatefulWidget {
  const StatusMobadaratRequest({super.key});

  @override
  State<StatusMobadaratRequest> createState() => _StatusMobadaratRequestState();
}

class _StatusMobadaratRequestState extends State<StatusMobadaratRequest> {
      ...
      floatingActionButton: BlocBuilder<GetProfileCubit, GetProfileState>(
        builder: (context, state) {
          if (state is GetProfileSuccessful) {
            return state.data.data.addMobadra == "yes"
                ? MaterialButton(
                    onPressed: () {
                      BlocProvider.of<BottomNavCubit>(context)
                          .updateBottomNavIndex(kStatusMobadaratRequest);
                      BlocProvider.of<BottomNavCubit>(context)
                          .navigationQueue
