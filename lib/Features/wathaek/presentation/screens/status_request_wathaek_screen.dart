...
class StatusWathaekRequest extends StatefulWidget {
  const StatusWathaekRequest({super.key});

  @override
  State<StatusWathaekRequest> createState() => _StatusWathaekRequestState();
}

class _StatusWathaekRequestState extends State<StatusWathaekRequest> {
      ...
      floatingActionButton: BlocBuilder<GetProfileCubit, GetProfileState>(
        builder: (context, state) {
          if (state is GetProfileSuccessful) {
            return state.data.data.addNashat == "yes"
                ? MaterialButton(
                    onPressed: () {
               
                      BlocProvider.of<BottomNavCubit>(context)
                          .updateBottomNavIndex(kStatusWathaekRequest);
                      BlocProvider.of<BottomNavCubit>(context)
                          .navigationQueue
